...
clf_rf = RandomForestClassifier(n_estimators=200, min_samples_leaf=3, max_features=0.5, n_jobs=-1)
...
for key, model in models.items():
    ...
    test, train, train_acc = f.fit_train_model(model, X, y, x_test, config.values['fold_type'], config.values['n_fold'])
    ...
    p_test.to_csv(config.values['output_data_path'] + 'test/results_' +
                    key.lower() + '.csv', index=False)
...
p_test[config.values['target_column']] = model.predict(df_test)
